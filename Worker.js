...
function mapVehicles(raw){
  var vehicles=[];
  try{
    var data=raw.data||raw,branches=data.branch||[];
    for(var i=0;i<branches.length;i++){
      var branch=branches[i],bv=branch.vehicles||[];
      for(var j=0;j<bv.length;j++){
        var v=bv[j];
        vehicles.push({imei:v.imei_no||"",id:v.imei_no||"",registration:v.vehicle_no||v.device_name||"",driver_name:v.driver_name||"Unknown",make:v.device_model||"",company:data.company_name||COMPANY,location:branch.branch_name||""});
      }
    }
  }catch(e){vehicles=[];}
...
async function getVehicleList(){
  var raw=await netstarPOST("/external/company-vehicles",{});
  return mapVehicles(raw).vehicles;
}

async function handleVehicles(){
  var raw=await netstarPOST("/external/company-vehicles",{});
  return json(mapVehicles(raw));
}
...
async function handleDriverScore(url){
  ...
  // get vehicle info so we have registration
  ...
  // match by driver name if available, otherwise use first record
  var r=list[0];
...
async function handleFleetScores(url){
  ...
  // get both vehicle list and driver perf in parallel
  ...
  var scored=list.map(function(r){
    ...
    // if no name match, try to find unmatched vehicle
  ...
  vlist.forEach(function(v){
    if(scoredImeis.indexOf(v.imei)<0){
      scored.push({
        ...
        company:v.company,location:v.location,make:v.make,
